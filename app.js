...
const minimal_args = [
  ...
  // SECURITY NOTE: --no-sandbox is required for Cloud Run deployment
  // Chrome's sandbox conflicts with container security model and prevents startup
  // see: https://cloud.google.com/run/docs/container-contract#sandbox
  // This is safe because: 
  // 1) input validation blocks malicious params, 
  // 2) we only load trusted ENS metadata service images,
  // 3) HTML is controlled/generated by us, 4) Cloud Run provides container isolation
  '--no-sandbox',
];
...
(async () => {
  ...
  await cluster.task(async ({ page, data: { resolution } }) => {
    const htmlContent = `
    <!DOCTYPE html>
    <html>
      <head>
